...
train.bi.mi <- apply(as.matrix(train.bi.dtm), 2, function(x,y) { mi.plugin(table(x,y)/length(y)) }, labels[index.train])
...
for(i in 1:length(mi)) {
  ...
  for(j in 1:k) {
    pred.cv <- createModels(suffix=".mi", train=(as.matrix(train.dtm)[,train.mi.order[1:mi[i]]])[folds!=j,], BiTrain=(as.matrix(train.bi.dtm)[,train.bi.mi.order[1:mi[i]]])[folds!=j,], test=(as.matrix(train.dtm)[,train.mi.order[1:mi[i]]])[folds==j, ], BiTest=(as.matrix(train.bi.dtm)[,train.bi.mi.order[1:mi[i]]])[folds==j,], labels=(labels[index.train])[folds!=j])
...
test.tf <- as.matrix(train.dtm) # Use training data instead of test data
...
train.bi.mat <- as.matrix(train.bi.dtm)
...
test.bi.tf <- as.matrix(train.bi.dtm) # Use training instead of test data
...
Now we make a comparison after which we select our final models. This time, however, we use bootstrap CV instead of 10-fold so to not bias our results.
...
colnames(best.mat) <- "Old Accuracy"
newbest <- numeric(8)

# First, we set up the data we need

# Tree bi: sparsity 0.99, stem
set.seed(37729)
k <- 10 #set the number of folds to 10
accvec.bs <- rep(0, k)
folds <- ceiling(runif(nrow(train.bi.dtm), min = 0, max = k))
accuracy <- function(obs, pred){
  sum(obs == pred) / length(obs)
}

# for(j in 1:k) {
#   tempTree <- rpart(classlabel~., data=data.frame(as.matrix(train.bi.dtm)[folds!=j,], classlabel=(labels[index.train])[folds!=j]), cp=0, method="class") #train on all but fold j
#   mod <- prune(tempTree, cp=cp.select(tempTree)) # Select best CP
#   pred <- predict(mod, newdata = data.frame(as.matrix(train.bi.dtm)[folds==j,]), type="class") #validate on fold j
...
Check to see if they have indeed improved!
