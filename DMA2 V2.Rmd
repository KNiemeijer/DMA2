...
It looks like for most features using the top 300 features based on mutual information provides the most improvement. We try one more possible improvement before comparing the results to check which algorithms work best with which feature selection.
...
train.tf <- DocumentTermMatrix(reviews.all[index.train], 
...
test.tf <- as.matrix(test.dtm) # Use training data instead of test data
for(i in 1:1114){test.tf[,i] <- test.tf[,i]*train.idf[i]}

train.bi.tf <-  DocumentTermMatrix(reviews.all[index.train], 
...
test.bi.tf <- as.matrix(test.bi.dtm) # Use training instead of test data
...
accvec.tf[,i] <- colMeans(accvec.cv)

rownames(accvec.tf) <- c("NB", "Log reg", "Tree", "RF", "NB Bi", "Log reg Bi", "Tree Bi", "RF Bi")
colnames(accvec.tf) <- "TF-IDF"
accvec.tf
cat("Best mutual information level is top", which.max(colMeans(accvec.tf)), "with average accuracy = ", max(colMeans(accvec.tf)))
```
